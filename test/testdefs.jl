...
using ChainRulesCore: rrule, unthunk, AbstractThunk, Thunk, NoTangent, ZeroTangent
...
if !isdefined(Main, :test_api)
    ...
    function test_fused_rrule(A)
        @testset "Test fused rrule for $(nameof(typeof(A)))" begin
            ...
            @test f_pullback(ZeroTangent()) == (NoTangent(), ZeroTangent())
        end
    end
    ...
    function test_splitview_rrule(A, smode)
        @testset "Test splitup rrule for $(nameof(typeof(A)))" begin
            ...
            @test unthunk(tangents[2]) == A
            @test f_pullback(ZeroTangent()) == (NoTangent(), ZeroTangent(), NoTangent())
            if !(smode isa NonSplitMode)
                # The cotangent may arrive as a plain nested array with
                # unknown split mode (the natural cotangent of generic AD):
                nat_cotangent = [collect(x) for x in A_split]
                @test unthunk(f_pullback(nat_cotangent)[2]) == A
            end
        end
    end
    ...
    function test_api(A, A_unsplit_ref)
        @testset "Test API for $(nameof(typeof(A)))" begin
            global g_state = (;A, A_unsplit_ref)
            ...
            if !isempty(A)
                ...
                if smode isa UnknownSplitMode
                    # The generic implementation maps over nested closures,
                    # which not all supported Julia versions can infer:
                    @test innermap(maptest_f, A) == innermap(maptest_f, Array(A))
                    @test deepmap(maptest_f, A) == deepmap(maptest_f, Array(A))
                else
