...
ArraysOfArrays._sa_with_eltype(::Type{SArray{S,U,N,L}}, ::Type{T}) where {S,U,N,L,T} = SArray{S,T,N,L}
ArraysOfArrays._sa_with_eltype(::Type{SVector{S}}, ::Type{T}) where {S,T} = SVector{S,T}
ArraysOfArrays._sa_with_eltype(::Type{SMatrix{M,N}}, ::Type{T}) where {M,N,T} = SMatrix{M,N,T,M * N}
ArraysOfArrays._sa_with_eltype(::Type{SArray{S}}, ::Type{T}) where {S<:Tuple,T} = SArray{S,T,tuple_length(S),tuple_prod(S)}

function ArraysOfArrays._sa_with_eltype(::Type{SA}, ::Type{T}) where {SA<:StaticArray,T}
...
ArraysOfArrays._sa_without_eltype(::Type{SArray{Tuple{S1},T,1,L}}) where {S1,T,L} = SVector{S1}
ArraysOfArrays._sa_without_eltype(::Type{SArray{Tuple{S1,S2},T,2,L}}) where {S1,S2,T,L} = SMatrix{S1,S2}
ArraysOfArrays._sa_without_eltype(::Type{SArray{S,T,N,L}}) where {S,T,N,L} = SArray{S}
ArraysOfArrays._sa_without_eltype(::Type{SA}) where {SA<:StaticArray} = SA
...
function ArraysOfArrays.splitup(A::AbstractArray{T}, ::StaticSlices{SA0}) where {T,SA0<:StaticArray}
    SA = ArraysOfArrays._sa_with_eltype(SA0, T)
...
# Disambiguation against the generic slices method:
@inline ArraysOfArrays._stacked_impl(A::Base.AbstractSlices{<:StaticArray}, smode::StaticSlices) = fused(A)
